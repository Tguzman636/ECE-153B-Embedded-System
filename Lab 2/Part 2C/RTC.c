...
void RTC_Init(void) {
	...
	RTC_Set_Calendar_Date(RTC_WEEKDAY_WEDNESDAY, 20, RTC_MONTH_JANUARY, 22); /* [TODO] These values are stubs - fill in current date */
	
	// Configure the Time 
	RTC_Set_Time(RTC_TR_PM, 5, 3, 2); /* [TODO] These values are stubs - fill in current time */
...
void RTC_Set_Calendar_Date(uint32_t WeekDay, uint32_t Day, uint32_t Month, uint32_t Year) {
	...
	date |= (__RTC_CONVERT_BIN2BCD(WeekDay) << RTC_POSITION_DR_WDU); // Weekday
	date |= (__RTC_CONVERT_BIN2BCD(Day) << RTC_POSITION_DR_DU);      // Day
	date |= (__RTC_CONVERT_BIN2BCD(Month) << RTC_POSITION_DR_MU);    // Month
	date |= (__RTC_CONVERT_BIN2BCD(Year) << RTC_POSITION_DR_YU);     // Year
...
void RTC_Set_Time(uint32_t Format12_24, uint32_t Hour, uint32_t Minute, uint32_t Second) {
	...
	time |= (Format12_24);   // Format 12/24
	time |= (__RTC_CONVERT_BIN2BCD(Hour) << RTC_POSITION_TR_HU);   // Hour
	time |= (__RTC_CONVERT_BIN2BCD(Minute) << RTC_POSITION_TR_MU); // Minute
	time |= (__RTC_CONVERT_BIN2BCD(Second) << RTC_POSITION_TR_SU); // Second
...
void RTC_Enable_Write_Protection(void) {
	// Write Key to RTC_WPR
	RTC->WPR |= RTC_WPR_KEY;
}
	/* 																	Get Time Format:
	* 				Read Tenth Value				Read Units Value					Shift Position LSB
	*/
