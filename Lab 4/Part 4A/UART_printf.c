...
int fputc(int ch, FILE *f) { 
	uint8_t c;
	c = ch & 0x00FF;
	USART_Write(USART2, (uint8_t *)&c, 1); // Comment out for part 1
	//USART_Write(USART1, (uint8_t *)&c, 1); // Comment out for part 2
	return(ch);
}
...
int fgetc(FILE *f) {  
	...
	//rxByte = USART_Read(USART1); // Comment out for part 2
	return rxByte;
}
